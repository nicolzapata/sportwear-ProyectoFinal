...
const crearUsuario = async (req, res) => {
  try {
    const { nombre, email, contrasena, id_rol, permiso_cuotas, tipo_doc, documento, telefono, ciudad, id_barrio, direccion } = req.body;
    if (!nombre || !email || !contrasena || !id_rol)
      return res.status(400).json({ message: 'Nombre, email, contraseña y rol son requeridos' });
    const usuario = await authService.crearUsuario({ nombre, email, contrasena, id_rol, permiso_cuotas, tipo_doc, documento, telefono, ciudad, id_barrio, direccion });
